...
const phlutsCoreRoot = path.join(repoRoot, 'packages', 'phluts_core', 'lib');

const widgetsExportPath = path.join(phlutsCoreRoot, 'widgets', 'widgets.dart');
...
async function parseWidgetCatalog() {
  ...
  for (const match of exportMatches) {
    ...
    const slugMatch = exportRel.match(/phluts_([^/]+)\.dart$/);
    if (!slugMatch) {
      continue;
    }

    const slug = slugMatch[1];
    const widgetFile = path.join(phlutsCoreRoot, 'widgets', exportRel);
    const widgetSource = await readText(widgetFile);

    const classMatch = widgetSource.match(/class\s+(Phluts[A-Za-z0-9_]+)\s+extends\s+PhlutsWidget/);
    if (!classMatch) {
      continue;
    }
