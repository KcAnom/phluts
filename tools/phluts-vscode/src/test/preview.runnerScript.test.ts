...
suite('Preview runner script', () => {
  test('buildRunnerScript includes function invocation', () => {
    const script = buildRunnerScript(
      '/tmp/phluts/screens/home.dart',
      'homeScreen',
    );

    assert.ok(script.includes("import 'file:///tmp/phluts/screens/home.dart' as target;"));
  ...
  test('writeRunnerArtifacts creates script and output paths', async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'phluts-vscode-preview-'));
    const sourceFile = path.join(workspace, 'phluts', 'home.dart');
