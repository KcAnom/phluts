...
export interface PhlutsExpressionRange {
  widgetName: string;
  startOffset: number;
  endOffset: number;
}
...
export function collectPhlutsExpressionRanges(text: string): PhlutsExpressionRange[] {
  const matches = [...text.matchAll(/\b(Phluts[A-Za-z0-9_]+)\s*\(/g)];
  const ranges: PhlutsExpressionRange[] = [];
...
export function findWrappableExpression(
  ...
  const ranges = collectPhlutsExpressionRanges(text).filter((item) => {
    if (isCursor) {
