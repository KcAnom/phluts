...
function registerWrapCommands(context: vscode.ExtensionContext) {
  ...
  const customDisposable = vscode.commands.registerCommand(
    ...
    async (uri?: vscode.Uri, range?: vscode.Range) => {
      ...
      // Select "PhlutsWidget" so user can type the widget/class name inline (Flutter-style)
  ...
  const disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      try {
        ...
        void vscode.window.showInformationMessage('Phluts catalog regenerated.');
      } catch (error) {
        void vscode.window.showErrorMessage(
          `Failed to regenerate Phluts catalog: ${String(error)}`,
        );
      }
    },
  );
