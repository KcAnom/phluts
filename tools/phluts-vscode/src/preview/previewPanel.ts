...
<body>
  ...
  <script>
    ...
    const hostEventTypes = new Set([
      'phluts.preview.ready',
      'phluts.preview.rendered',
      'phluts.preview.error',
    ]);
    ...
    retryButton.addEventListener('click', () => {
      vscode.postMessage({ type: 'phluts.preview.retry' });
    });

    themeSelect.addEventListener('change', () => {
      const value = themeSelect.value || null;
      vscode.postMessage({ type: 'phluts.preview.selectTheme', themeName: value });
    });
      ...
      if (frame.contentWindow) {
        try {
          frame.contentWindow.postMessage(
            { type: 'phluts.preview.setPlatform', platform: platformMap[device] || 'web' },
            '*',
          );
        } catch (_) {}
      }
    ...
    function startDelivery() {
      ...
      deliveryTimer = setInterval(() => {
        ...
        if (attempts >= maxAttempts) {
          stopDelivery();
          vscode.postMessage({ type: 'phluts.preview.error', message: 'Preview delivery timed out' });
    ...
    window.addEventListener('message', (event) => {
      ...
      if (
        ...
        if (hostMessage.type === 'phluts.preview.ready') {
          hostReady = true;
          clearTimeout(iframeReadyTimeout);
          iframeReloadCount = 0;
          latestDeliveredRequestId = null;
        ...
        if (hostMessage.type === 'phluts.preview.rendered') {
      ...
      if (message.type === 'phluts.preview.state') {
      ...
      if (message.type === 'phluts.preview.themes') {
      ...
      if (message.type === 'phluts.preview.render') {
      ...
      if (message.type === 'phluts.preview.loadFonts') {
        postRenderToFrame(message);
      }
    });

    vscode.postMessage({ type: 'phluts.preview.webview.ready' });
  </script>
</body>
</html>`;
}
