...
const SCREEN_DECLARATION_REGEX =
  /@PhlutsScreen\s*\(\s*screenName\s*:\s*(['"])([^'"]+)\1[\s\S]*?\)\s*PhlutsWidget\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)/g;
...
export function discoverScreens(document: vscode.TextDocument): ScreenDescriptor[] {
  ...
  for (const match of text.matchAll(SCREEN_DECLARATION_REGEX)) {
    ...
    const functionSignatureIndex = fullMatch.indexOf(`PhlutsWidget ${functionName}`);
...
export async function pickScreenDescriptor(
  ...
  const selected = await vscode.window.showQuickPick(
    ...
    {
      title: 'Select Phluts screen for preview',
      placeHolder: 'Pick a @PhlutsScreen target',
    },
  );
