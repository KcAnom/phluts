...
export class PreviewManager implements vscode.Disposable {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.outputChannel = vscode.window.createOutputChannel('Phluts Preview');
  }
  ...
  async openPreview(): Promise<void> {
    ...
    if (!settings.enabled) {
      void vscode.window.showInformationMessage(
        'Phluts preview is disabled by phlutsVscode.preview.enable.',
      );
      return;
    }
    ...
    if (!editor || editor.document.languageId !== 'dart') {
      void vscode.window.showErrorMessage('Open a Dart file containing @PhlutsScreen to preview.');
      return;
    }
  ...
  async refreshPreview(): Promise<void> {
    ...
    void vscode.window.showErrorMessage('No active Phluts screen document to refresh.');
  }
  ...
  async selectScreen(): Promise<void> {
    ...
    if (screens.length === 0) {
      void vscode.window.showErrorMessage('No @PhlutsScreen declarations found in this file.');
      return;
    }
  ...
  private async handleDidSaveDocument(document: vscode.TextDocument): Promise<void> {
    ...
    // Re-discover themes if the saved file contains @PhlutsThemeRef (new or existing)
    // to pick up newly added themes without a full restart.
    const fileText = document.getText();
    const mightContainTheme = fileText.includes('@PhlutsThemeRef');
  ...
  private async runRefreshLoop(): Promise<void> {
    ...
    while (this.pendingRefresh) {
      ...
      try {
        ...
        void vscode.window.showErrorMessage(`Phluts preview failed: ${String(error)}`);
      }
    }
    this.refreshRunning = false;
  }
  ...
  private async refreshDocument(
    ...
    const screens = discoverScreens(document);
    if (screens.length === 0) {
      throw new Error('No @PhlutsScreen declarations found in this document.');
    }
  ...
  private async handleWebviewMessage(message: PreviewWebviewMessage): Promise<void> {
    if (message.type === 'phluts.preview.retry') {
      await this.refreshPreview();
      return;
    }

    if (message.type === 'phluts.preview.webview.ready') {
    ...
    if (message.type === 'phluts.preview.selectTheme') {
  ...
  private handlePreviewHostEvent(message: PreviewOutboundMessage) {
    if (message.type === 'phluts.preview.ready') {
    ...
    if (message.type === 'phluts.preview.rendered') {
    ...
    if (message.type === 'phluts.preview.error') {
      if (
        message.requestId
        && this.lastRenderRequestId
    ...
    if (message.type === 'phluts.preview.log') {
  ...
  private getSettings(): PreviewSettings {
    ...
    return {
      ...
      buildCommand: config.get<string>(
        ...
        'phluts build --project "${projectFolder}"',
      ),
