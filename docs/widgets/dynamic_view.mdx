...
| ------------- | ---------------------- | -------- | --------------------------------------------------------------------- |
| request       | `PhlutsNetworkRequest`   | Yes      | API request configuration (url, method, headers, etc.)                |
| template      | `PhlutsWidget`           | Yes      | Template to render with data from the API response                    |
...
| loaderWidget  | `PhlutsWidget`           | No       | Custom widget to display while loading data                           |
| errorWidget   | `PhlutsWidget`           | No       | Custom widget to display when an error occurs                         |
| itemTemplate  | `PhlutsWidget`           | No       | Template to render each item in a list of items from the API response |
| emptyTemplate | `PhlutsWidget`           | No       | Template to render when the API response contains an empty list       |
...
PhlutsDynamicView(
  ...
  template: PhlutsText(data: 'Hello, {{name}}!'),
)
```
        ...
        child: PhlutsColumn(
          ...
          children: [
            ...
            PhlutsText(
              ...
              style: PhlutsTextStyle(fontSize: 24, fontWeight: PhlutsFontWeight.w700),
            ),
            PhlutsSizedBox(height: 8),
            PhlutsText(
              data: 'Email: {{email}}',
              style: PhlutsTextStyle(fontSize: 16, color: PhlutsColors.grey),
            ),
            PhlutsText(
              data: 'Phone: {{phone}}',
              style: PhlutsTextStyle(fontSize: 16, color: PhlutsColors.grey),
...
PhlutsDynamicView(
  ...
  targetPath: 'products',
  template: PhlutsGridView(
    crossAxisCount: 2,
    itemTemplate: PhlutsCard(
      child: PhlutsColumn(
        children: [
          PhlutsImage(src: '{{image_url}}'),
          PhlutsText(data: '{{name}}'),
          PhlutsText(data: '\${{price}}'),
        ],
      ),
    ),
  ),
  emptyTemplate: PhlutsCenter(
    child: PhlutsColumn(
      mainAxisSize: PhlutsMainAxisSize.min,
      children: [
        PhlutsIcon(icon: 'shopping_cart', size: 64, color: PhlutsColors.grey),
        PhlutsSizedBox(height: 16),
        PhlutsText(
          data: 'No products available',
          style: PhlutsTextStyle(fontSize: 18, fontWeight: PhlutsFontWeight.bold, color: PhlutsColors.grey),
        ),
        PhlutsSizedBox(height: 8),
        PhlutsText(
          data: 'Check back later for new products!',
          style: PhlutsTextStyle(fontSize: 14, color: PhlutsColors.grey),
        ),
        PhlutsSizedBox(height: 24),
        PhlutsElevatedButton(
          child: PhlutsText(data: 'Refresh'),
          onPressed: PhlutsNavigateAction(route: '/refresh'),
        ),
      ],
    ),
  ),
...
PhlutsDynamicView(
  ...
  template: PhlutsColumn(
    children: [
      PhlutsText(data: 'Items loaded: {{length}}'),
    ],
  ),
)
```
...
PhlutsDynamicView(
  ...
  template: PhlutsCard(
    child: PhlutsColumn(
      children: [
        PhlutsText(data: '{{product.name}}'),
        PhlutsText(data: 'Price: \${{product.price}}'),
      ],
    ),
  ),
)
...
PhlutsDynamicView(
  ...
  template: PhlutsText(data: 'Data loaded successfully!'),
)
```
...
PhlutsDynamicView(
  ...
  template: PhlutsText(data: 'Featured Post: {{title}}'),
)
```
