---
title: "RadioGroup"
description: "Documentation for RadioGroup"
---

import { PLAYGROUND_BASE_URL } from "/snippets/playground_base.mdx";

export const radioGroupPreviewJson = {
  "type": "radioGroup",
  "child": {
    "type": "column",
    "children": [
      {
        "type": "listTile",
        "leading": {
          "type": "radio",
          "radioType": "adaptive",
          "value": "1",
          "groupValue": "1"
        },
        "title": {
          "type": "text",
          "data": "Male",
          "align": "center",
          "style": {
            "fontSize": 21
          }
        }
      },
      {
        "type": "listTile",
        "leading": {
          "type": "radio",
          "radioType": "adaptive",
          "value": "2",
          "groupValue": "1"
        },
        "title": {
          "type": "text",
          "data": "Female",
          "align": "center",
          "style": {
            "fontSize": 21
          }
        }
      },
      {
        "type": "listTile",
        "leading": {
          "type": "radio",
          "radioType": "adaptive",
          "value": "3",
          "groupValue": "1"
        },
        "title": {
          "type": "text",
          "data": "Other",
          "align": "center",
          "style": {
            "fontSize": 21
          }
        }
      }
    ]
  }
};
export const radioGroupPreviewSrc = `${PLAYGROUND_BASE_URL}/embed`;

The Phluts RadioGroup allows you to build Flutter Radio widgets using JSON. 
To know more about the Radio widget in Flutter, refer to the [official documentation](https://api.flutter.dev/flutter/material/Radio-class.html).

## RadioGroup Properties

| Property   | Type                   | Description                                                |
|------------|------------------------|------------------------------------------------------------|
| id         | `String`               | The id will be used to save the selected value of radio.   |
| groupValue | `String`               | The currently selected value for a group of radio buttons. |
| child      | `PhlutsWidget`           | The child of radioGroup.                                   |

## Radio Properties

| Property                   | Type                        | Description                                                                                                     |
|----------------------------|-----------------------------|-----------------------------------------------------------------------------------------------------------------|
| radioType                  | `PhlutsRadioType`             | The type of radio.                                                                                              |
| value                      | `String`                    | The value represented by this radio.                                                                            |
| mouseCursor                | `PhlutsMouseCursor`           | The cursor for a mouse pointer when it enters or is hovering over the radio.                                    |
| toggleable                 | `bool`                      | Set to true if this wanted to deselect when selected.                                                           |
| activeColor                | `PhlutsColor`                 | The color to use when this radio button is selected.                                                            |
| inactiveColor              | `PhlutsColor`                 | The color to use when this radio button is not selected.                                                        |
| fillColor                  | `PhlutsColor`                 | The color that fills the radio button, in all WidgetStates.                                                     |
| focusColor                 | `PhlutsColor`                 | The color for the radio's Material when it has the input focus.                                                 |
| hoverColor                 | `PhlutsColor`                 | The color for the radio's Material when a pointer is hovering over it.                                          |
| overlayColor               | `PhlutsColor`                 | The color for the radio's Material.                                                                             |
| splashRadius               | `double`                    | The splash radius of the circular Material ink response.                                                        |
| materialTapTargetSize      | `PhlutsMaterialTapTargetSize` | Configures the minimum size of the tap target.                                                                  |
| visualDensity              | `PhlutsVisualDensity`         | Defines how compact the radio's layout will be.                                                                 |
| autofocus                  | `bool`                      | True if this widget will be selected as the initial focus when no other node in its scope is currently focused. |
| useCheckmarkStyle          | `bool`                      | Controls whether the radio displays in a checkbox style or the default iOS radio style.                         |
| useCupertinoCheckmarkStyle | `bool`                      | Controls whether the checkmark style is used in an iOS-style radio.                                             |
| enabled                    | `bool`                      | Whether this radio is enabled for user interaction.                                                             |
| backgroundColor            | `PhlutsColor`                 | The background color of the radio.                                                                               |
| side                       | `PhlutsBorderSide`            | The border side of the radio.                                                                                    |
| innerRadius                | `double`                    | The inner radius of the radio in logical pixels.                                                                |

## Example

<Tabs sync={false}>
  <Tab title="Dart">

```dart
PhlutsRadioGroup(
  child: PhlutsColumn(
    children: [
      PhlutsListTile(
        leading: PhlutsRadio(
          radioType: PhlutsRadioType.adaptive,
          value: '1',
          groupValue: '1',
        ),
        title: PhlutsText(
          data: 'Male',
          textAlign: PhlutsTextAlign.center,
          style: PhlutsTextStyle(fontSize: 21),
        ),
      ),
      PhlutsListTile(
        leading: PhlutsRadio(
          radioType: PhlutsRadioType.adaptive,
          value: '2',
          groupValue: '1',
        ),
        title: PhlutsText(
          data: 'Female',
          textAlign: PhlutsTextAlign.center,
          style: PhlutsTextStyle(fontSize: 21),
        ),
      ),
      PhlutsListTile(
        leading: PhlutsRadio(
          radioType: PhlutsRadioType.adaptive,
          value: '3',
          groupValue: '1',
        ),
        title: PhlutsText(
          data: 'Other',
          textAlign: PhlutsTextAlign.center,
          style: PhlutsTextStyle(fontSize: 21),
        ),
      ),
    ],
  ),
)
```

  </Tab>
  <Tab title="JSON">

```json
{
  "type": "radioGroup",
  "child": {
    "type": "column",
    "children": [
      {
        "type": "listTile",
        "leading": {
          "type": "radio",
          "radioType": "adaptive",
          "value": "1",
          "groupValue": "1"
        },
        "title": {
          "type": "text",
          "data": "Male",
          "align": "center",
          "style": {
            "fontSize": 21
          }
        }
      },
      {
        "type": "listTile",
        "leading": {
          "type": "radio",
          "radioType": "adaptive",
          "value": "2",
          "groupValue": "1"
        },
        "title": {
          "type": "text",
          "data": "Female",
          "align": "center",
          "style": {
            "fontSize": 21
          }
        }
      },
      {
        "type": "listTile",
        "leading": {
          "type": "radio",
          "radioType": "adaptive",
          "value": "3",
          "groupValue": "1"
        },
        "title": {
          "type": "text",
          "data": "Other",
          "align": "center",
          "style": {
            "fontSize": 21
          }
        }
      }
    ]
  }
}
```

  </Tab>
  <Tab title="Preview">
    <Frame>
      <iframe
        id="phluts"
        src={radioGroupPreviewSrc}
        title="Phluts Playground"
        className="w-full rounded-xl border-0"
        style={{ height: "640px" }}
        loading="lazy"
        onLoad={(event) => {
          const iframe = event.currentTarget;
          const targetOrigin = PLAYGROUND_BASE_URL;
          const message = {
            type: "phluts-preview-json",
            payload: radioGroupPreviewJson
          };

          let attempts = 0;
          const maxAttempts = 12;
          const interval = setInterval(() => {
            iframe.contentWindow?.postMessage(message, targetOrigin);
            attempts += 1;

            if (attempts >= maxAttempts) {
              clearInterval(interval);
            }
          }, 250);
        }}
      />
    </Frame>
  </Tab>
</Tabs>

