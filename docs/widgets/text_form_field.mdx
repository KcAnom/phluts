...
The Phluts TextFormField allows you to build a Flutter TextFormField widget using JSON.
...
| decoration                    | `PhlutsInputDecoration`           | The decoration to show around the text form field.                                                                                                 |
| initialValue                  | `String`                        | The initial value of the text form field.                                                                                                          |
| keyboardType                  | `PhlutsTextInputType`             | The type of keyboard to use for editing the text.                                                                                                  |
| textInputAction               | `PhlutsTextInputAction`           | The action button to use for the keyboard.                                                                                                         |
| textCapitalization            | `PhlutsTextCapitalization`        | How the text should be capitalized. Defaults to `TextCapitalization.none`.                                                                         |
| style                         | `PhlutsTextStyle`                 | The style to use for the text.                                                                                                                     |
| textAlign                     | `PhlutsTextAlign`                 | How the text should be aligned. Defaults to `TextAlign.start`.                                                                                     |
| textAlignVertical             | `PhlutsTextAlignVertical`         | How the text should be aligned vertically.                                                                                                         |
...
| smartDashesType               | `PhlutsSmartDashesType`           | The type of smart dashes to use.                                                                                                                   |
| smartQuotesType               | `PhlutsSmartQuotesType`           | The type of smart quotes to use.                                                                                                                   |
| maxLengthEnforcement          | `PhlutsMaxLengthEnforcement`      | How the max length limit should be enforced.                                                                                                       |
| expands                       | `bool`                          | Whether the text form field should expand to fill its parent. Defaults to `false`.                                                                 |
| keyboardAppearance            | `PhlutsBrightness`                | The appearance of the keyboard.                                                                                                                    |
| scrollPadding                 | `PhlutsEdgeInsets`                | The amount of space by which to inset the text form field when scrolling. Defaults to `PhlutsEdgeInsets(bottom: 20, top: 20, left: 20, right: 20)`. |
...
| cursorColor                   | `PhlutsColor`                     | The color of the cursor.                                                                                                                           |
| hintText                      | `String`                        | The hint text to display.                                                                                                                          |
| autovalidateMode              | `PhlutsAutovalidateMode`          | The mode to use for autovalidation.                                                                                                                |
| inputFormatters               | `List<PhlutsInputFormatter>`      | The list of input formatters to apply to the text. Defaults to an empty list.                                                                      |
| validatorRules                | `List<PhlutsFormFieldValidator>`  | The list of validator rules to apply to the text. Defaults to an empty list.                                                                       |
...
Use `validatorRules` to validate user input. Each entry is a `PhlutsFormFieldValidator` with a `rule`, an optional `message` shown when validation fails, and an optional `options` map for parameterized rules.
...
PhlutsTextFormField(
  ...
  inputFormatters: [
    PhlutsInputFormatter(
      type: PhlutsInputFormatterType.mask,
      rule: r'\d',
      mask: '##/##/####',
    ),
  ],
  style: PhlutsTextStyle(fontSize: 16, fontWeight: PhlutsFontWeight.w400, height: 1.5),
  decoration: PhlutsInputDecoration(
    labelText: 'Date of birth',
    hintText: 'DD/MM/YYYY',
    floatingLabelBehavior: 'always',
    filled: true,
    fillColor: PhlutsColors.white,
    border: PhlutsOutlineInputBorder(borderRadius: 8, color: '#24151D29'),
  ),
)
```
