...
Phluts SelectableText allows you to build the Flutter SelectableText widget using JSON.
...
| onTap                      | `PhlutsAction?`        | The action to perform when the text is tapped.    |
...
PhlutsSelectableText(
  data: 'This is a selectable text',
  style: PhlutsTextStyle(fontSize: 18, fontWeight: PhlutsFontWeight.w600),
  showCursor: true,
  cursorColor: PhlutsColors.red,
)
```
