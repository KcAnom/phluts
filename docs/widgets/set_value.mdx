...
The Phluts SetValue widget allows you to set multiple key-value pairs in the application's state and optionally render a child widget. It's useful for managing application state through JSON configuration.
...
| child    | `PhlutsWidget`                  | Optional child widget to render after the values are set.        |
...
Use `PhlutsSetValue` to initialize state when a screen loads:
...
After setting values with `PhlutsSetValue`, you can read them using placeholder syntax `{{key}}`:
...
PhlutsSetValue(
  ...
  child: PhlutsText(data: '{{greeting}}, {{user.name}}!'),
)
```
...
<Note>
For action-based state updates (e.g., on button press), see [PhlutsSetValueAction](/actions/set_value).
</Note>
