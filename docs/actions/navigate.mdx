...
The **Navigate** action performs navigation in a Flutter app from JSON or Dart. For an overview of how navigation works in Phluts (destinations, `PhlutsNavigator`, and when to use each option), see [Navigation in Phluts](/concepts/navigation_in_phluts). For Flutter’s Navigator model, see the [official documentation](https://api.flutter.dev/flutter/widgets/Navigator-class.html).
...
Network request used to load the destination screen JSON. The response body is parsed as Phluts widget JSON and shown as the new route. Use for server-driven screens loaded at navigation time.

**Used with:** `navigationStyle` = `push`, `pushReplacement`, or `pushAndRemoveAll`.

See [Network Request Action](/actions/network_request) for `PhlutsNetworkRequest` fields (url, method, headers, body, etc.).
...
Inline Phluts widget tree (JSON map) used as the destination screen. No network or asset load; the screen is defined entirely in the action. Good for one-off or dynamic screens.
...
Path to a local asset file (e.g. in `assets/`) that contains the Phluts widget JSON for the destination screen. The file is loaded and parsed when navigating. Good for bundled or offline screens.
...
- With **Phluts screens** (push, pushReplacement, pushAndRemoveAll): name of the Phluts screen (e.g. from your `/phluts` folder or Phluts Cloud). The Phluts runtime resolves it to a widget (e.g. via `Phluts(routeName: routeName)`).
...
**Used with:** any push-style `navigationStyle`. For Flutter routes use `pushNamed`, `pushReplacementNamed`, or `pushNamedAndRemoveAll`; for Phluts screens use `push`, `pushReplacement`, or `pushAndRemoveAll`.
...
| Value | Description |
|-------|-------------|
| `push` | Pushes a new route on top of the current one. Use with `request`, `widgetJson`, `assetPath`, or `routeName` (Phluts screen). |
...
Arguments passed to the new route. For **Phluts screens** (push/pushReplacement/pushAndRemoveAll + routeName), the Phluts runtime can use these when building the screen. For **Flutter named routes** (pushNamed, pushReplacementNamed, pushNamedAndRemoveAll), this is the `arguments` object passed to `Navigator.pushNamed(..., arguments: arguments)`.

For examples (Dart and JSON), see [Navigation in Phluts](/concepts/navigation_in_phluts#examples).
