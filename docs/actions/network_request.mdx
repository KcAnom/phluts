...
The `PhlutsNetworkRequest` class is used to make network requests in Phluts applications.
...
| results         | `List<PhlutsNetworkResult>` | The list of results to handle different status codes. Defaults to an empty list. |

### Method

The `Method` enum defines the different HTTP methods that can be used in the Phluts Network Request Action.
...
The `PhlutsNetworkResult` class is used to handle different status codes in the Phluts Network Request Action.
