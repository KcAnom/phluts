---
title: "Phluts Registry"
description: "Understand the central hub for widget parsers, action parsers, and global state in the Phluts ecosystem"
---

The **Phluts Registry** is the "brain" of the Phluts framework. It acts as a centralized, in-memory repository that manages how the engine translates static JSON data into live Flutter widgets and functional actions.

## 1. What is Phluts Registry?

The Phluts Registry is a singleton service that serves as a lookup table for the rendering engine. It manages three primary areas:
- **Widget Parsers**: Mapping of JSON `type` strings (e.g., `"text"`) to their respective `PhlutsParser`.
- **Action Parsers**: Mapping of logic triggers (e.g., `"navigate"`) to their `PhlutsActionParser`.
...
- **Extend Phluts**: Register custom widget or action parsers that are not included in the core library.
...
## 3. How it fits into Phluts Architecture
...
2. **Parser Discovery**: The Phluts engine queries the Registry for the parser matching the JSON's `type` field.
...
PhlutsRegistry.instance.setValue('userName', 'Alex Smith');
```
...
If you want to change how a standard widget like `PhlutsText` behaves globally (e.g., to force all text to be uppercase or add a custom logging interceptor), you can override the default parser.

```dart
PhlutsRegistry.instance.register(
