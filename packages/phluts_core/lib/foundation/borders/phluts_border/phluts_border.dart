import 'package:json_annotation/json_annotation.dart';
import 'package:phluts_core/core/phluts_widget.dart';
import 'package:phluts_core/foundation/borders/phluts_border_side/phluts_border_side.dart';
import 'package:phluts_core/foundation/colors/phluts_color/phluts_colors.dart';

part 'phluts_border.g.dart';

/// Border style options for drawing borders.
enum PhlutsBorderStyle {
  /// Skip the border.
  none,

  /// Draw the border as a solid line.
  solid,

  // if you add more, think about how they will lerp
}

/// A Phluts representation of borders for UI elements.
///
/// This class supports both uniform borders (applied to all sides) and
/// individual side borders with different styling. You can specify global
/// properties or customize each side individually.
///
/// {@tool snippet}
/// Dart Example:
/// ```dart
/// const PhlutsBorder(
///   color: PhlutsColors.blue,
///   borderStyle: PhlutsBorderStyle.solid,
///   width: 2.0,
///   strokeAlign: 0.0,
/// )
/// ```
/// {@end-tool}
///
/// {@tool snippet}
/// JSON Example:
/// ```json
/// {
///   "color": "#2196F3",
///   "borderStyle": "solid",
///   "width": 2.0,
///   "strokeAlign": 0.0
/// }
/// ```
/// {@end-tool}
@JsonSerializable()
class PhlutsBorder implements PhlutsElement {
  /// Creates a border with optional uniform and individual side properties.
  const PhlutsBorder({
    this.color,
    this.borderStyle,
    this.width,
    this.strokeAlign,
    this.top,
    this.right,
    this.bottom,
    this.left,
  });

  /// The color applied to all sides (when not using individual sides).
  final PhlutsColor? color;

  /// The border style applied to all sides (when not using individual sides).
  final PhlutsBorderStyle? borderStyle;

  /// The width applied to all sides (when not using individual sides).
  final double? width;

  /// The stroke alignment applied to all sides (when not using individual sides).
  final double? strokeAlign;

  /// The top border side with individual styling.
  final PhlutsBorderSide? top;

  /// The right border side with individual styling.
  final PhlutsBorderSide? right;

  /// The bottom border side with individual styling.
  final PhlutsBorderSide? bottom;

  /// The left border side with individual styling.
  final PhlutsBorderSide? left;

  /// Creates a uniform border applied to all sides.
  ///
  /// This factory method creates a border with the same styling applied
  /// to all four sides (top, right, bottom, left).
  ///
  /// {@tool snippet}
  /// Dart Example:
  /// ```dart
  /// PhlutsBorder.all(
  ///   color: PhlutsColors.blue,
  ///   width: 2.0,
  ///   borderStyle: PhlutsBorderStyle.solid,
  /// )
  /// ```
  /// {@end-tool}
  ///
  /// {@tool snippet}
  /// JSON Example:
  /// ```json
  /// {
  ///   "color": "#2196F3",
  ///   "width": 2.0,
  ///   "borderStyle": "solid"
  /// }
  /// ```
  /// {@end-tool}
  factory PhlutsBorder.all({
    PhlutsColor? color,
    PhlutsBorderStyle? borderStyle,
    double? width,
    double? strokeAlign,
  }) {
    return PhlutsBorder(
      color: color,
      borderStyle: borderStyle,
      width: width,
      strokeAlign: strokeAlign,
    );
  }

  /// Creates a symmetric border with different styling for horizontal and vertical sides.
  ///
  /// This factory method creates a border where horizontal sides (left, right)
  /// have the same styling, and vertical sides (top, bottom) have the same styling.
  ///
  /// {@tool snippet}
  /// Dart Example:
  /// ```dart
  /// PhlutsBorder.symmetric(
  ///   horizontal: PhlutsBorderSide(
  ///     color: PhlutsColors.blue,
  ///     width: 2.0,
  ///     borderStyle: PhlutsBorderStyle.solid,
  ///   ),
  ///   vertical: PhlutsBorderSide(
  ///     color: PhlutsColors.red,
  ///     width: 1.0,
  ///     borderStyle: PhlutsBorderStyle.solid,
  ///   ),
  /// )
  /// ```
  /// {@end-tool}
  ///
  /// {@tool snippet}
  /// JSON Example:
  /// ```json
  /// {
  ///   "top": {"color": "#F44336", "width": 1.0, "borderStyle": "solid"},
  ///   "bottom": {"color": "#F44336", "width": 1.0, "borderStyle": "solid"},
  ///   "left": {"color": "#2196F3", "width": 2.0, "borderStyle": "solid"},
  ///   "right": {"color": "#2196F3", "width": 2.0, "borderStyle": "solid"}
  /// }
  /// ```
  /// {@end-tool}
  factory PhlutsBorder.symmetric({
    PhlutsBorderSide? horizontal,
    PhlutsBorderSide? vertical,
  }) {
    return PhlutsBorder(
      top: vertical,
      bottom: vertical,
      left: horizontal,
      right: horizontal,
    );
  }

  /// Creates a [PhlutsBorder] from a JSON map.
  factory PhlutsBorder.fromJson(Map<String, dynamic> json) =>
      _$PhlutsBorderFromJson(json);

  /// Converts this [PhlutsBorder] instance to a JSON map.
  @override
  Map<String, dynamic> toJson() => _$PhlutsBorderToJson(this);
}
