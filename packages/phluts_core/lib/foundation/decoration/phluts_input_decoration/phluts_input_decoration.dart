import 'package:json_annotation/json_annotation.dart';
import 'package:phluts_core/core/phluts_widget.dart';
import 'package:phluts_core/foundation/borders/phluts_input_border/phluts_input_border.dart';
import 'package:phluts_core/foundation/colors/phluts_color/phluts_colors.dart';
import 'package:phluts_core/foundation/geometry/phluts_edge_insets/phluts_edge_insets.dart';
import 'package:phluts_core/foundation/text/phluts_text_style/phluts_text_style.dart';

part 'phluts_input_decoration.g.dart';

/// Phluts model that represents Flutter's [InputDecoration].
///
/// Provides labels, hints, helper/error text, and optional prefix/suffix
/// widgets for inputs like [TextField].
///
/// {@tool snippet}
/// Dart Example:
/// ```dart
/// PhlutsInputDecoration(
///   labelText: 'Email',
///   hintText: 'name@example.com',
///   prefixText: '@',
/// )
/// ```
/// {@end-tool}
///
/// {@tool snippet}
/// JSON Example:
/// ```json
/// {
///   "labelText": "Email",
///   "hintText": "name@example.com",
///   "prefixText": "@"
/// }
/// ```
/// {@end-tool}
///
/// See also:
///  * Flutter's [InputDecoration documentation](https://api.flutter.dev/flutter/material/InputDecoration-class.html)
@JsonSerializable(explicitToJson: true)
class PhlutsInputDecoration extends PhlutsElement {
  /// Creates an input decoration with the specified properties.
  const PhlutsInputDecoration({
    this.icon,
    this.labelText,
    this.labelStyle,
    this.floatingLabelBehavior,
    this.hintText,
    this.hintStyle,
    this.helperText,
    this.helperStyle,
    this.errorText,
    this.errorStyle,
    this.prefixIcon,
    this.prefixText,
    this.prefixStyle,
    this.suffixIcon,
    this.suffixText,
    this.suffixStyle,
    this.isDense,
    this.contentPadding,
    this.filled,
    this.fillColor,
    this.alignLabelWithHint,
    this.errorBorder,
    this.focusedBorder,
    this.focusedErrorBorder,
    this.disabledBorder,
    this.enabledBorder,
    this.border,
  });

  /// A widget to display before the decoration's container.
  final PhlutsWidget? icon;

  /// Optional label text to display above/beside the input.
  final String? labelText;

  /// Text style for [labelText].
  final PhlutsTextStyle? labelStyle;

  /// Controls when the label floats above the input.
  ///
  /// Supported values mirror Flutter's [FloatingLabelBehavior]: `auto`,
  /// `always`, and `never`.
  final String? floatingLabelBehavior;

  /// Optional placeholder text.
  final String? hintText;

  /// Text style for [hintText].
  final PhlutsTextStyle? hintStyle;

  /// Optional helper and error texts.
  final String? helperText;

  /// Text style for [helperText].
  final PhlutsTextStyle? helperStyle;

  /// Error text to display below the input field.
  final String? errorText;

  /// Text style for [errorText].
  final PhlutsTextStyle? errorStyle;

  /// Optional prefix/suffix widgets and texts.
  /// Widget to display before the input content.
  final PhlutsWidget? prefixIcon;

  /// Optional text to display before the input content.
  final String? prefixText;

  /// Text style for [prefixText].
  final PhlutsTextStyle? prefixStyle;

  /// A widget to display after the editable part of the text field.
  final PhlutsWidget? suffixIcon;

  /// Optional text to display after the editable part of the text field.
  final String? suffixText;

  /// Text style for [suffixText].
  final PhlutsTextStyle? suffixStyle;

  /// Whether the decoration uses less vertical space.
  final bool? isDense;

  /// Padding for the decoration's container.
  final PhlutsEdgeInsets? contentPadding;

  /// Whether the decoration's background is filled with [fillColor].
  final bool? filled;

  /// Background color used when [filled] is true.
  final PhlutsColor? fillColor;

  /// Whether to align the floating label with the input's hint/center.
  /// Useful for multi-line inputs so the label isn't vertically centered.
  final bool? alignLabelWithHint;

  /// Border to show when the input has an error.
  final PhlutsInputBorder? errorBorder;

  /// Border to show when the input is focused.
  final PhlutsInputBorder? focusedBorder;

  /// Border to show when the input is focused and has an error.
  final PhlutsInputBorder? focusedErrorBorder;

  /// Border to show when the input is disabled.
  final PhlutsInputBorder? disabledBorder;

  /// Border to show when the input is enabled and not focused.
  final PhlutsInputBorder? enabledBorder;

  /// Default border (used when no other border is specified).
  final PhlutsInputBorder? border;

  /// Creates a [PhlutsInputDecoration] from a JSON map.
  ///
  /// The [json] argument must be a valid JSON representation of a
  /// [PhlutsInputDecoration].
  factory PhlutsInputDecoration.fromJson(Map<String, dynamic> json) =>
      _$PhlutsInputDecorationFromJson(json);

  /// Converts this [PhlutsInputDecoration] instance to a JSON map.
  @override
  Map<String, dynamic> toJson() => _$PhlutsInputDecorationToJson(this);
}
