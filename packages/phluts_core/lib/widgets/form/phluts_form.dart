import 'package:json_annotation/json_annotation.dart';
import 'package:phluts_core/phluts_core.dart';

part 'phluts_form.g.dart';

/// A Phluts model representing a form scope used to coordinate input fields and actions.
///
/// This widget establishes a [PhlutsFormScope] at runtime so input widgets can
/// register and retrieve values (e.g., via the `getFormValue` action), and so
/// actions like validation can be performed consistently across fields.
///
/// {@tool snippet}
/// Dart Example:
/// ```dart
/// PhlutsForm(
///   autovalidateMode: PhlutsAutovalidateMode.onUserInteraction,
///   child: PhlutsColumn(children: [
///     PhlutsTextFormField(id: 'username'),
///     PhlutsTextFormField(id: 'password'),
///   ]),
/// )
/// ```
/// {@end-tool}
///
/// {@tool snippet}
/// JSON Example:
/// ```json
/// {
///   "type": "form",
///   "autovalidateMode": "always",
///   "child": {
///     "type": "column",
///     "children": [
///       { "type": "textFormField", "id": "username" },
///       { "type": "textFormField", "id": "password" }
///     ]
///   }
/// }
/// ```
/// {@end-tool}
///
/// See also:
///  * Flutter's [Form documentation](https://api.flutter.dev/flutter/widgets/Form-class.html)
@JsonSerializable()
class PhlutsForm extends PhlutsWidget {
  /// Creates a [PhlutsForm].
  const PhlutsForm({this.autovalidateMode, this.child});

  /// The mode to control auto validation of fields within the form.
  final PhlutsAutovalidateMode? autovalidateMode;

  /// The widget subtree to display inside the form scope.
  final PhlutsWidget? child;

  /// Widget type identifier.
  @override
  String get type => WidgetType.form.name;

  /// Creates a [PhlutsForm] from a JSON map.
  factory PhlutsForm.fromJson(Map<String, dynamic> json) =>
      _$PhlutsFormFromJson(json);

  /// Converts this [PhlutsForm] instance to a JSON map.
  @override
  Map<String, dynamic> toJson() => _$PhlutsFormToJson(this);
}
