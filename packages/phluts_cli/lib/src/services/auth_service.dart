...
import 'package:phluts_cli/src/utils/oauth_pkce.dart';
...
class AuthService {
  // Production OAuth credentials (provided by Phluts team)
  // Note: In production, these would be the actual Phluts OAuth app credentials
  ...
  Future<void> login() async {
    try {
      ...
      if (existingToken != null && !existingToken.isExpired) {
        ...
        ConsoleLogger.info(
          'Run "phluts logout" first if you want to login with a different account.',
        );
        return;
      }
  ...
  Future<void> status() async {
    try {
      ...
      if (isAuthenticated) {
        ...
        ConsoleLogger.info(
          'Not authenticated. Run "phluts login" to authenticate.',
        );
      }
    } catch (e) {
