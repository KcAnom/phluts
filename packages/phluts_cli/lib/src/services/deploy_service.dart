...
/// Service for deploying Phluts JSON files to the cloud
class DeployService {
  final HttpClientService _httpClient = HttpClientService.instance;

  /// Deploy all built JSON files (from phluts/.build) to the Screens API
  /// - Reads projectId from lib/default_phluts_options.dart
  /// - For each {name}.json in phluts/.build, POST to Cloud Function "screens"
  Future<void> deploy({String? projectPath}) async {
    final projectDir = projectPath ?? Directory.current.path;

    // Read projectId from default_phluts_options.dart
    final projectId = await _readProjectIdFromOptions(projectDir);
    if (projectId == null || projectId.isEmpty) {
      throw PhlutsException(
        'Could not determine projectId from lib/default_phluts_options.dart. Run "phluts init" first.',
      );
    }

    // Build output directory produced by BuildService
    final buildDirPath = path.join(projectDir, 'phluts', '.build');
    final buildDir = Directory(buildDirPath);
    if (!await buildDir.exists()) {
      throw PhlutsException(
        'Build directory not found at $buildDirPath. Run "phluts build" first.',
      );
    }
    ...
    if (totalFailures == 0) {
      final consoleUrl = 'https://console.phluts.dev/project/$projectId';
      ConsoleLogger.info(
        'Open your project in the Phluts Console to inspect your screens and themes: $consoleUrl',
      );
    }
  }
  ...
  Future<void> _uploadScreen({
    ...
    try {
      ...
      throw PhlutsException('Failed to upload screen "$screenName": $e');
    }
  }
  ...
  Future<void> _uploadTheme({
    ...
    try {
      ...
      throw PhlutsException('Failed to upload theme "$themeName": $e');
    }
  }

  /// Extract projectId from lib/default_phluts_options.dart
