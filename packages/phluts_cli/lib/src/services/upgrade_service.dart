...
import '../exceptions/phluts_exception.dart';
...
/// Service for upgrading the Phluts CLI
class UpgradeService {
  static const String _repo = 'PhlutsDev/cli-installer';
  ...
  static const String _publicKeyEnv = 'PHLUTS_UPGRADE_PUBLIC_KEY_B64';
  ...
  Future<String> getCurrentVersion() async {
    try {
      final result = await Process.run(
        Platform.isWindows ? 'phluts.exe' : 'phluts',
        ['--version'],
        runInShell: false,
      );
      if (result.exitCode == 0) {
        final output = result.stdout.toString().trim();
        // Parse "phluts_cli version: X.Y.Z" or "phluts_cli version: X.Y.Z-dev"
  ...
  Future<String> getDownloadUrlForVersion(String version) async {
    ...
    if (osArch == null) {
      throw PhlutsException('Unsupported platform');
    }

    final tag = 'phluts-cli-v$version';
    final os = osArch['os']!;
    ...
    final assetName = 'phluts_cli_${version}_${os}_$arch.$extension';
  ...
  Future<Map<String, dynamic>> _getLatestRelease() async {
    ...
    try {
      ...
      request.headers.set('User-Agent', 'phluts-cli');
      ...
      final version = tagName.replaceFirst('phluts-cli-v', '');
      ...
      if (downloadUrl == null || binaryAssetName == null) {
        throw PhlutsException('No compatible binary found for your platform');
      }
  ...
  Future<void> _downloadAndInstall(
    ...
    final tempDir = await Directory.systemTemp.createTemp('phluts_upgrade_');
    final archivePath = path.join(
      tempDir.path,
      isWindows ? 'phluts.zip' : 'phluts.tar.gz',
    );
    ...
    try {
      ...
      final checksumPath = path.join(tempDir.path, 'phluts.sha256');
      ...
      final checksumSigPath = path.join(tempDir.path, 'phluts.sha256.sig');
      ...
      // Copy the binary
      final binaryName = isWindows ? 'phluts.exe' : 'phluts';
  ...
  Future<void> _extractTarGz(String archivePath, String destDir) async {
    ...
    if (result.exitCode != 0) {
      throw PhlutsException('Failed to extract archive: ${result.stderr}');
    }
  }
  ...
  String _parseSha256(String checksumFileContents) {
    ...
    if (parts.isEmpty || parts.first.isEmpty) {
      throw PhlutsException('Invalid checksum file format');
    }
    return parts.first;
  }
  ...
  List<int> _parseBase64Bytes({
    ...
    try {
      ...
      throw PhlutsException('Invalid base64 value for $context');
    }
  }

  String _getInstallDir() {
    // Check if PHLUTS_INSTALL_DIR is set
    final envDir = Platform.environment['PHLUTS_INSTALL_DIR'];
    if (envDir != null && envDir.isNotEmpty) {
      return envDir;
    }

    // Default to ~/.phluts/bin
    if (Platform.isWindows) {
      final userProfile = Platform.environment['USERPROFILE'] ?? '';
      return path.join(userProfile, '.phluts', 'bin');
    } else {
      final home = Platform.environment['HOME'] ?? '';
      return path.join(home, '.phluts', 'bin');
    }
  }
}
