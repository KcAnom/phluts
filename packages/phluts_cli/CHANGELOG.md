...
- fix: Update workflow to fix missing required environment variable `PHLUTS_BASE_API_URL`.
- fix: Re-enable phluts_cli release workflow and update package and dependency versions.
- feat: Enable compile-time environment variable injection for CLI release builds.

## 1.5.0

- feat: Open-source Phluts CLI.
- feat: Add phluts_cli release workflow.
- feat: Add `DynamicDataProvider` and `TemplateBuilder`.
- feat: Migrate linting from `flutter_lints` to `lints/recommended`.
- refactor: Convert `phluts_logger` to a pure Dart package.
