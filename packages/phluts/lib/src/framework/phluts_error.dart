...
/// Structured information about a Phluts parsing or runtime error.
///
/// This class encapsulates details about errors that occur when parsing
/// JSON to Phluts widgets/actions or during runtime execution. It provides
/// context like the failing entity type, original JSON payload, and stack trace
/// to help with debugging and error reporting.
///
/// Used primarily with [PhlutsErrorWidgetBuilder] to display custom error UI
/// when Phluts parsing fails.
///
/// Example usage:
/// ```dart
/// // Creating a PhlutsError
/// final error = PhlutsError(
///   type: 'container',
...
/// ```
class PhlutsError {
  /// Creates a [PhlutsError] with the given details.
  ///
...
/// A widget that displays detailed error information when Phluts fails to parse JSON.
///
...
/// Use a custom [PhlutsErrorWidgetBuilder] if you need to display stack traces.
///
/// Example:
/// ```dart
/// PhlutsErrorWidget(
...
/// )
/// ```
class PhlutsErrorWidget extends StatefulWidget {
  const PhlutsErrorWidget({super.key, required this.errorDetails});

  final PhlutsError errorDetails;

  @override
  State<PhlutsErrorWidget> createState() => _PhlutsErrorWidgetState();
}

/// Expandable section types for better state management.
enum _ExpandableSection { json }

class _PhlutsErrorWidgetState extends State<PhlutsErrorWidget> {
  ...
  String _getTroubleshootingTips() {
    ...
    if (errorStr.contains('type') && errorStr.contains('not found')) {
      return '• Check if the widget/action type is registered\n'
          ...
          '• Ensure the parser is added in Phluts.initialize()';
    }
    ...
    return '• Check the JSON structure matches widget requirements\n'
        ...
        '• Consult the widget documentation at docs.phluts.dev';
  }
}
