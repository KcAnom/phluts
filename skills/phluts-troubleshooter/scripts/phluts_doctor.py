#!/usr/bin/env python3
"""Run baseline health checks for Phluts projects."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run diagnostics for a Phluts project.")
...
def has_annotation(phluts_dir: Path) -> bool:
    for dart_file in phluts_dir.rglob("*.dart"):
        if re.search(r"@PhlutsScreen\s*\(", read(dart_file)):
            return True
    return False
...
def main() -> int:
    ...
    options_dart = root / "lib" / "default_phluts_options.dart"
    phluts_dir = root / "phluts"
    gitignore = root / ".gitignore"
    build_dir = root / "phluts" / ".build"
    ...
    if phluts_dir.exists() and phluts_dir.is_dir():
        add_result(results, "pass", "phluts-directory", str(phluts_dir))
        found = has_annotation(phluts_dir)
        add_result(
            results,
            "pass" if found else "fail",
            "phluts-screen-annotation",
            "Found @PhlutsScreen in phluts/" if found else "No @PhlutsScreen found in phluts/",
        )
    else:
        add_result(results, "fail", "phluts-directory", str(phluts_dir))
        add_result(results, "fail", "phluts-screen-annotation", "phluts/ missing")

    if main_dart.exists():
        has_initialize = "Phluts.initialize(" in read(main_dart)
        add_result(
            results,
            "pass" if has_initialize else "fail",
            "phluts-initialize",
            "Phluts.initialize call found" if has_initialize else "Phluts.initialize call missing",
        )
    ...
    if gitignore.exists():
        ...
        add_result(
            ...
            "build-ignore",
            ".build ignore rule present" if ignores_build else "Consider ignoring phluts/.build",
        )
    else:
