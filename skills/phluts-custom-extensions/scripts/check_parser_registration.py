#!/usr/bin/env python3
"""Check that a parser class appears in the same file as Phluts.initialize."""
...
def main() -> int:
    ...
    init_index = content.find("Phluts.initialize(")
    parser_index = content.find(args.parser_class)

    if init_index < 0:
        print("[FAIL] Phluts.initialize(...) call not found")
        return 1
