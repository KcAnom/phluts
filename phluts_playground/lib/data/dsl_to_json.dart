/// Converts Phluts DSL source text into the widget JSON the runtime renders.
///
/// The real compiler (`phluts build`) runs the analyzer, executes the annotated
/// `@PhlutsScreen` function in a Dart process and calls `toJson()` on the result.
...
/// string interpolation, methods on non-Phluts values — throws
...
/// Widget classes (those extending `PhlutsWidget`) — these serialize with a
/// `"type"` discriminator. Everything else (`PhlutsTextStyle`, `PhlutsBoxDecoration`
/// …) is a value object and serializes as a bare map. Generated from phluts_core.
const Set<String> _widgetClasses = {
  ...
  'PhlutsVisibility',
  'PhlutsWrap',
  // From the phluts_webview plugin rather than phluts_core, but it is a PhlutsWidget
...
String _typeForClass(String className) {
  ...
  final name = className.substring('Phluts'.length);
  return name[0].toLowerCase() + name.substring(1);
}

/// A top-level helper function the screen can call, e.g.
/// `PhlutsWidget _socialRow({required String icon}) { return PhlutsRow(…); }`.
class _FunctionDef {
...
Map<String, dynamic> dslToJson(String source) {
  ...
  if (value is! Map<String, dynamic>) {
    throw const DslParseException('The screen must return a Phluts widget.');
  }
  return value;
}
...
Map<String, _FunctionDef> _extractFunctions(String stripped) {
  ...
  // `PhlutsWidget _socialRow({required String icon}) {`
...
class _DslParser {
  ...
  /// Handles `PhlutsText(...)`, `PhlutsEdgeInsets.only(...)`, `Enum.value`, and a
  /// trailing `.toJson()`.
  dynamic _parseIdentifierExpression() {
    ...
    if (!hasArgs) {
      ...
      // Enum or static constant: `PhlutsFontWeight.w600`, `double.maxFinite`.
      return memberName;
    }
  ...
  dynamic _buildJson(String className, String? constructor, _Arguments args) {
    // `PhlutsEdgeInsets.all(8)` / `PhlutsBorderRadius.circular(8)` → 8, which
    ...
    // `PhlutsEdgeInsets.symmetric(horizontal: h, vertical: v)`.
    if (className == 'PhlutsEdgeInsets' && constructor == 'symmetric') {
    ...
    // `PhlutsAction(jsonData: {...})` is the escape hatch for a raw action map,
    // and it serializes as that map rather than as a wrapper around it.
    if (className == 'PhlutsAction') {
