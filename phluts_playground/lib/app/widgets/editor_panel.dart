...
import 'package:phluts_playground/app/widgets/find_panel.dart';
import 'package:phluts_playground/app/widgets/section_nav.dart';
import 'package:phluts_playground/data/dsl_to_json.dart';
...
Mode _patchDartMode() {
  langDart.contains?.insertAll(0, [
    ...
    // Call expressions — `helloPhluts(`, `.only(`, `.all(` → function yellow.
    Mode(
    ...
    // Type / constructor usages (UpperCamelCase) — `PhlutsText` → class teal.
    Mode(className: 'title.class', begin: r'\b[A-Z][A-Za-z0-9_]*'),
  ]);
  return langDart;
}

/// Minimal context menu for the code editor, mirroring the Phluts Console.
...
class _CodeEditorContentState extends State<_CodeEditorContent> {
  ...
  /// VS Code Dark+–style colors, same as the Phluts Console editor.
...
/// Segmented Dart/JSON switcher: Dart shows the Phluts DSL source, JSON the
/// deployed output that `phluts build` generates from it.
  ...
  Widget _segment(
    ...
    return Tooltip(
      message: value == CodeLanguage.dart
          ? 'Phluts DSL source (compiled to JSON by phluts build)'
