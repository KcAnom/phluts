...
@PhlutsScreen(screenName: 'demo')
PhlutsWidget demo() {
  return PhlutsScaffold(
    body: PhlutsCenter(child: PhlutsText(data: 'edited from Dart')),
  );
}
''';
...
@PhlutsScreen(screenName: 'demo')
PhlutsWidget demo() {
  return PhlutsScaffold(body: _row(label: 'from helper'));
}

PhlutsWidget _row({required String label}) {
  return PhlutsRow(children: [PhlutsText(data: label)]);
}
''';
  ...
  test('the built-in screens preview too', () {
    // hello_phluts and form_screen carry their DSL inline on the cubit rather
    ...
    for (final entry in {
      'hello_phluts': helloPhlutsDartCode,
...
@PhlutsScreen(screenName: 'demo')
PhlutsWidget demo() {
  return PhlutsScaffold(body: PhlutsText(data: r'costs $100'));
}
''';
...
@PhlutsScreen(screenName: 'demo')
PhlutsWidget demo() {
  return PhlutsScaffold(body: someRuntimeWidget);
}
''';
  ...
  test('half-typed Dart holds the preview without nagging', () {
    final result = parseEditorSource(
      'import 1;\n@PhlutsScreen(screenName: 1)\nPhlutsWidget d() { return Phluts',
      isDart: true,
    );
