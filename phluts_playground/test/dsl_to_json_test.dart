...
void main() {
  ...
  test('parsed DSL produces a renderable widget tree', () {
    ...
    for (final file in files) {
      ...
      try {
        // The preview hands this straight to Phluts.fromJson, so it has to be
  ...
  test('examples that round-trip exactly keep doing so', () {
    ...
    // `PhlutsEdgeInsets.all(12)` → `12`, which fromJson expands. The set may
...
List<String> _untypedWidgets(dynamic node, String path) {
  ...
  if (node is Map) {
    for (final entry in node.entries) {
      ...
      // payload on PhlutsNetworkRequest.
